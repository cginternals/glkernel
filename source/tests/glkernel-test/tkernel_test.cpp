...
TEST_F(tkernel_test, tkernel_indexed_value_access)
{
    ...
    EXPECT_EQ(64, fkernel.size());
    EXPECT_EQ( 2, fkernel.width());
    EXPECT_EQ( 4, fkernel.height());
    EXPECT_EQ( 8, fkernel.depth());
...
TEST_F(tkernel_test, tkernel_data_access)
{
    ...
    EXPECT_EQ(64, fkernel.size());
    EXPECT_EQ( 8, fkernel.width());
    EXPECT_EQ( 2, fkernel.height());
    EXPECT_EQ( 4, fkernel.depth());

    // check if a value set in kernel via spatial reference 
    // equals the value at the expected index ...

    const auto findex = fkernel.index(2, 1, 3);
    EXPECT_EQ(8 * 2 * 3 + 8 * 1 + 2, findex);
...
TEST_F(tkernel_test, tkernel_reset)
{
    ...
    EXPECT_EQ(1024, fkernel.size());
    EXPECT_EQ(1024, fkernel.width());
    EXPECT_EQ(   1, fkernel.height());
...
TEST_F(tkernel_test, tkernel1_defaults)
{
    ...
    EXPECT_EQ(1, fkernel.size());
    EXPECT_EQ(1, fkernel.width());
    EXPECT_EQ(1, fkernel.height());
    EXPECT_EQ(1, fkernel.depth());
    ...
    EXPECT_EQ(1, dkernel.size());
    EXPECT_EQ(1, dkernel.width());
    EXPECT_EQ(1, dkernel.height());
    EXPECT_EQ(1, dkernel.depth());
